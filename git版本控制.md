...
- git push的输出信息的最后一行含义
    - 格式：`<oldref>..<newref> fromref -> toref`
    - `oldref`推送前所指向的引用
    - `newref`推送后所指向的引用
    - `fromref`将要被推送的本地引用的名字
    - `toref`将要被更新的远程引用的名字

- `git log --no-merge issue54..origin/master`
    - `issue54..origin/master`是日志过滤器，表示只显示所有在后面分支(本例中的origin/master)但不在前面分支(本例中的issue54)的提交

- 显示来自当前主题分支与master分支的共同祖先起，该分支中的工作
    - `git diff master...contrib`


## GitHub

- GitHub使用用户邮件地址区分Git提交
    - 如果在自己的提交中使用了多个邮件地址，希望Git可以将包含了这些地址的任意版本库中的任意提交信息链接到自己账号下，需要在管理页面的Email部分添加你拥有的所有邮箱地址

- 派生项目(fork)
    - 如果你想要参与某个项目，但是并没有推送权限，这时可以对这个项目进行派生(fork)。得到一个完全属于你的项目副本，并具有推送权限
    - 将修改推送到派生出的项目副本中，并通过创建拉取请求(pull requset, 即PR)来让改动进入源版本库
    - 创建拉取请求之后，就会开启一个可供审查代码的版块，项目的拥有者和贡献者可以在此讨论相关修改，直到项目拥有者对其感到满意，并且认为这些修改可以被合并到版本库
...
- GitHub流程
    ...
    6. 讨论，根据实际情况继续修改
    7. 项目的拥有者合并或关闭你的拉取请求
    8. 将更新后的master分支同步到你的派生中

- 实际上GitHub在服务器上把合并请求分支视为一种"假分支"，默认情况下克隆时不会得到他们，但是他们隐式存在
    - `git ls-remote xxxxx` // 可以查看到这些引用
